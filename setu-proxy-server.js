...
// Debug — test credentials with a real Setu call
app.get("/debug/token", async (req, res) => {
  try {
    // Try fetching consent list — confirms credentials work
    const data = await setuRequest("GET", "/consents?limit=1");
    res.json({ success: true, message: "Setu credentials working ✓", sample: data });
  } catch (e) {
