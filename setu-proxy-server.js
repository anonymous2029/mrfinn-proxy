...
async function getToken() {
  ...
  // Setu OAuth2 token endpoint (sandbox)
  const tokenUrl = "https://auth-dev.setu.co/auth/realms/collect/protocol/openid-connect/token";

  const params = new URLSearchParams();
  params.append("grant_type",    "client_credentials");
  params.append("client_id",     SETU.clientId);
  params.append("client_secret", SETU.clientSecret);
  ...
  console.log("Token raw response:", text.slice(0, 300));

  let data;
  try { data = JSON.parse(text); }
  catch (e) { throw new Error("Token response is not JSON: " + text.slice(0, 200)); }

  // Setu returns "access_token" in OAuth response
